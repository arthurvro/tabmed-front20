...
function HomeRecepcao() {
    ...
    return (
        <div className="container">
            ...
            <button onClick={criarUsuario} className="btn-create">Criar Novo Usuário</button>
            <ul className="user-list">
                {usuarios.map(usuario => (
                    <li key={usuario.id} className="user-item">
                        <span>{usuario.nome} {usuario.sobrenome} - {usuario.tipoAcesso === 1 ? 'Médico' : usuario.tipoAcesso === 2 ? 'Recepção' : 'Cliente'}</span>
                        <div className="user-actions">
                            <button onClick={() => editarUsuario(usuario.id)} className="btn-edit">Editar</button>
                            <button onClick={() => ativarDesativarUsuario(usuario.id)} className="btn-toggle">
